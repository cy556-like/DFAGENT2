...
3. 🔴 **禁止在对话中输出完整 D0-D8 内容**——报告内容在生成的 xlsx/docx 文件中，对话中仅返回简短摘要（≤5 行：文件路径 + 模板匹配结果 + 5Why 路径概述）

4. 🔴 **末尾调用 generate_8d_report_tool 一次性生成 xlsx + docx**（参数：product/defect/customer/defect_rate/batch_size/template），展示下载链接
5. 输出顺序：简短摘要 → 调用 generate_8d_report_tool → 展示下载链接
...
🔴 **只有用户消息中包含以下关键词时才设 `auto_fill=True`**：
- "示例" / "例子" / "范例" / "样例" / "随便填" / "你帮我填" / "其他不要问我" / "不要问我" / "不用问我"

**除此之外一律 False**（包括"你帮我处理""你帮我分析""生成8D报告"等）。判断标准：用户是否明确表达「不想自己填、要AI帮忙填充全部」。

启用后脚本会自动填充：D1 团队姓名（化名）、联系方式（分机号）、D3/D5/D6/D7 责任人和完成时间、D8 签名/日期等（Excel 和 Word 都会填）。

🔴 **关键约束**：判断启用时必须在调用 `generate_8d_report_tool` 时真的传入 `auto_fill=True`，不能光在回复里说。
...
- ✅ 正确流程示例：先输出简短摘要"8D 报告已生成，使用 paint-defect 模板，5Why 路径指向涂装工艺参数异常 → 漆面颗粒。\n📥 [xlsx文件路径]\n📥 [docx文件路径]"→ 最后调用 **generate_8d_report_tool** 生成专业 xlsx+docx → 展示下载链接
