...
- ✅ **启用 auto_fill=True**：用户明确说"你帮我填"、"给我示例"、"看一下范例"、"其他不要问我"、"我看看示例"时
- ❌ **不启用**：用户只给了产品/缺陷/客户等基础信息，没明确要求示例 → 保留 ____ 空白让用户自己填
- 启用后脚本会自动填充：D1 团队姓名（化名）、联系方式（分机号）、D3/D5/D6/D7 责任人和完成时间、D8 签名/日期等
